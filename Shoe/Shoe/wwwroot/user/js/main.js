...
(function ($) {
    ...
    proQty.on('click', '.qtybtn', function () {
        ...
        var cartItemID = $button.parent().parent().find('.cartItemID').val();
        var productID = $button.parent().parent().find('.productID').val();
        var productPrice = $button.parent().parent().parent().parent().find('.shoping__cart__price').text();
        var price = productPrice.slice(0, productPrice.length - 1);
        $.ajax({
            ...
            success: function (data) {
                console.log("success");
                var priceItem = newVal * price;
                $button.parent().parent().parent().parent().find('.shoping__cart__total').text(priceItem + "đ");
                updateTotalPrice();
            }
        });
    });
